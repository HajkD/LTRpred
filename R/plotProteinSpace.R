#' @title Genome Size vs. Protein Space
#' @description Compute and visualize the correlation between genome size vs. Protein Space.
...
plotProteinSpace <- function(genome.matrix,
                            cor.method          = "spearman",
                            label.organism      = TRUE,
                            smooth.method       = "glm",
                            se                  = TRUE,
                            xlab                = "Protein coding gene length in Mega [bp]",
    ...
    if (cl.analysis) {
        ...
        cor.value <-
            stats::cor(genome.matrix$ProtSpaceMbp,
                       genome.matrix$genome.size,
                       method = cor.method)
        
        if (!arrow_lab) {
            res <- ggplot2::ggplot(genome.matrix,
                                   ggplot2::aes(x = ProtSpaceMbp,
                                                y = genome.size)) + 
                ggplot2::geom_point() +
