#' @title Plot correlation between #cluster members and any other variable
#' @description Visualize the correlation between the Cluster Number generated 
#' by \code{LTRpred} and any other variable 
#' and color each putative LTR transposon by it's e.g. superfamily name.
#' @param data \code{data.frame} returned by \code{LTRpred}.
#' @param y column name in \code{data} (lazy evaluation) that shall be correlated with \code{cn_3ltr} (copy number of predicted 3' LTR).
#' @param color.by color points by a column name in \code{data} (lazy evaluation).
...
plotCluster <-
    function(data,
        ...
        if (!is.element(color.by, c("fam", "dfam_target_name")))
            stop("Please specify either 'color.by = 'dfam_target_name' or 'color.by = 'fam' (in case 'chop.names = TRUE').", call. = FALSE)
        
        if (chop.names && (color.by != "fam"))
            stop("When specifying 'chop.names = FALSE' please also specify 'color.by = 'dfam_target_name'.'", call. = FALSE)
        ...
        if (chop.names)
            data <- dplyr::mutate(data, fam = unlist(sapply(dfam_target_name,fam_replace)))
        
        
        cl_names <- names(table(data$Clust_Cluster))
        cl_names <- cl_names[which(cl_names != "unique")]

        cluster.df <- do.call(rbind,lapply(cl_names, function(x) {
            
            LTRpred::cluster.members(data, x)
            
        }))
        
        cluster.df.final <- rbind(cluster.df, dplyr::filter(data, Clust_Cluster == "unique"))
