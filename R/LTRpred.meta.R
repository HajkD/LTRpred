...
LTRpred.meta <- function(genome.folder       = NULL, 
  ...
  if (!is.null(genome.folder) && is.null(result.folder) && !is.null(LTRpred.meta.folder)){
    ...
    cat("\n")
    if (quality.filter)
      cat("Apply filters: [ similarity >= ",sim,"% ] ; [ PBS or Protein Match ] ; [ #ORFs >= ",n.orfs,"]")
    ...
    for (i in 1:length(folders0)){
      ...
      print(file.path(LTRpred.meta.folder,folders0[i],
                      paste0(paste0(choppedFolder[-length(choppedFolder)],collapse = "_"),"_LTRpred_DataSheet.csv")))
      cat("\n")
      cat("\n")
      
      if (!file.exists(file.path(LTRpred.meta.folder,folders0[i],
                                 paste0(paste0(choppedFolder[-length(choppedFolder)],collapse = "_"),"_LTRpred_DataSheet.csv")))){
        
        print(paste0("Skip :",file.path(LTRpred.meta.folder,folders0[i],
                                        paste0(paste0(choppedFolder[-length(choppedFolder)],collapse = "_"),"_LTRpred_DataSheet.csv")), " -> folder was empty!"))
        cat("\n")
      } else {
        
        pred <- readr::read_delim(file.path(LTRpred.meta.folder,folders0[i],
                                            paste0(paste0(choppedFolder[-length(choppedFolder)],collapse = "_"),"_LTRpred_DataSheet.csv")), delim = ";")
        ...
        binned.similarities <- cut(pred$ltr_similarity,
                                   rev(seq(100,sim,-cut.range)),
                                   include.lowest = TRUE,
                                   right = TRUE)
        
        SimMatrix[i] <- list(table(factor(binned.similarities, 
    ...
    if (!is.null(file.name)){
      ...
      write.table(SimMatrix,paste0(file.name,"_SimilarityMatrix.csv"), 
                  sep       = ";", 
                  quote     = FALSE, 
                  col.names = TRUE, 
                  row.names = FALSE)
      
      write.table(GenomeInfo,paste0(file.name,"_GenomeInfo.csv"), 
                  sep       = ";", 
                  quote     = FALSE, 
                  col.names = TRUE, 
                  row.names = FALSE)
    ...
    if (is.null(file.name)){
      # store results in working directory
      write.table(SimMatrix,paste0(basename(LTRpred.meta.folder),"_SimilarityMatrix.csv"), 
                  sep       = ";", 
                  quote     = FALSE, 
                  col.names = TRUE, 
                  row.names = FALSE)
      
      write.table(GenomeInfo,paste0(basename(LTRpred.meta.folder),"_GenomeInfo.csv"), 
                  sep       = ";", 
                  quote     = FALSE, 
                  col.names = TRUE, 
                  row.names = FALSE)
    ...
    if (is.null(file.name)){
      # store results in working directory
      write.table(SimMatrix,paste0(basename(result.folder),"_SimilarityMatrix.csv"),
                  sep       = ";", 
                  quote     = FALSE, 
                  col.names = TRUE, 
                  row.names = FALSE)
      
      write.table(GenomeInfo,paste0(basename(result.folder),"_GenomeInfo.csv"),
                  sep       = ";",
                  quote     = FALSE,
                  col.names = TRUE,
                  row.names = FALSE)
