...
LTRpred.meta <- function(genome.folder       = NULL,
  ...
  if (!is.null(genome.folder) &&
    ...
    genomes <-
      genomes[!stringr::str_detect(genomes, "md5cheksum")]
    ...
    for (i in 1:length(folders0)) {
      ...
      if (!file.exists(file.path(
        ...
        gs[i] <-
          sum(as.numeric(genome.size@ranges@width)) / 1000000
        ...
        genome.quality[i] <-
          sum(as.numeric(Biostrings::vcountPattern("N", genome.size))) / sum(as.numeric(genome.size@ranges@width))
      }
    }
    ...
    SimMatrix <-
      data.frame(organism = genomes.chopped[available.genomes], SimMatrix)
    ...
    for (i in 1:length(folders0)) {
      ...
      if (quality.filter) {
        # try to reduce false positives by filtering for PBS and ORFs and rel #N's in TE <= 0.1
        pred <- quality.filter(pred, sim = sim, n.orfs = n.orfs, strategy = strategy)
