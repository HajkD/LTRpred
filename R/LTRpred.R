...
#' @param cluster shall predicted transposons be clustered with \code{\link{CLUST}}.
#' @param clust.sim cluster reject if sequence similarity is lower than this threshold when performing clustering with \code{\link{CLUST}}.
#' @param cluster.file file path to pre-computed clustering file generated by \code{\link{CLUSTpred}}.
#' @param copy.number.est shall copy number estimation (including solo LTR prediction) be performed? Default is \code{copy.number.est = TRUE}.
    ...
    if (!is.null(LTRdigestOutput)) {
        ...
        if (!is.null(genome.file)) {
            ...
            if (cluster) {
                ...
                if (!is.null(cluster.file)){
                    # read cluster output generated by CLUSTpred()
                    cluster.file <- read.uc(cluster.file)
                }
