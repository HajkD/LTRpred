#' @title Copy Number Quantification of predicted LTR transposons
#' @description Detect copies of predicted LTR transposons and report genomic locations of corresponding copies using a BLAST search strategy.
...
te.cn <- function(LTRpred.folder, 
    ...
    if (is.null(output)){
        output <- file.path(tempdir(),"te_blast_output.txt")
    }
    ...
    LTR.fasta <- file.path(LTRpred.folder,ltrdigest.folder,paste0(folder.name,"-ltrdigest_complete.fas"))

    LTR.filtered.fasta <- file.path(LTRpred.folder,ltrdigest.folder,paste0(folder.name,"-ltrdigest_complete_",ltr.similarity,".fas"))
    ...
    system( paste0("blastn -query ",ws.wrap.path(LTR.filtered.fasta)," -subject ",ws.wrap.path(genome),
                   " -out ", ws.wrap.path(output) ," ","-evalue ", eval," -max_target_seqs ",max.hits," -dust no -outfmt '6 qseqid sseqid pident nident 
    ...
    BLASTOutput <- dplyr::mutate(BLASTOutput, scope = abs(s_len - alig_length) / s_len)
    # filter for potentially (biologically meaningful) solo LTR hits
    BLASTOutput <- dplyr::filter(BLASTOutput, scope >= scope.cutoff, perc_identity >= perc.ident.cutoff) 
