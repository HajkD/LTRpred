...
#' @param genome.folder path to the folder storing all unmasked genomes for which \code{\link{LTRpred}} based de novo LTR retrotransposon prediction shall be performed.
...
#' @param ... additional arguments passed to \code{\link{PlotSizeCorrelation}}.
#' @author Hajk-Georg Drost
#' @importFrom magrittr %>%
#' @export

PlotJumperSizeCorrelation <- function(genome.folder, 
                                      genome.matrix, 
                                      ltr.similarity = 98,
                                      sci.name.len   = 1,
                                      name.sep       = "_",
  ...
  meta.summarize.table <- meta.summarize(genome.folder, ltr.similarity)
  gm.jumpers <- meta.summarize.table %>% dplyr::group_by(organism) %>% dplyr::summarise(nLTRs = n())
  
  if (sci.name.len == 1){
    gm.jumpers$organism <- unlist(sapply(gm.jumpers$organism, function(x) {
  ...
  gm.jumpers.joined <- dplyr::inner_join(dplyr::select(genome.matrix,-nLTRs),gm.jumpers, by = "organism")
  
  PlotSizeCorrelation(genome.matrix = gm.jumpers.joined, main = main, ...)
}