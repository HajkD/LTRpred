...
ltr.cn <- function(data.sheet,
    ...
    LTRpred.tbl <- read.ltrpred(data.sheet)
    ...
    LTR.filtered.fasta_3ltr <- file.path(tempdir(),paste0(folder.name,"-ltrdigest_3ltr_",ltr.similarity,".fas"))
    LTR.filtered.fasta_5ltr <- file.path(tempdir(),paste0(folder.name,"-ltrdigest_5ltr_",ltr.similarity,".fas"))
    ...
    system( paste0("makeblastdb -in ",ws.wrap.path(genome), " -dbtype nucl -hash_index") )

    # BLAST putative LTRs against genome file for 3 prime LTR
    system( paste0("blastn -query ",ws.wrap.path(LTR.filtered.fasta_3ltr)," -db ",ws.wrap.path(genome),
                   " -out ", ws.wrap.path(output_3ltr) ," ","-evalue ", eval," -max_target_seqs ",max.hits," -num_threads ",cores," -dust no -outfmt '6 qseqid sseqid pident nident 
                   length mismatch gapopen gaps positive ppos qstart qend qlen sstart send slen evalue bitscore score'")
    )
    
    # BLAST putative LTRs against genome file for 5 prime LTR
    system( paste0("blastn -query ",ws.wrap.path(LTR.filtered.fasta_5ltr)," -db ",ws.wrap.path(genome),
                   " -out ", ws.wrap.path(output_5ltr) ," ","-evalue ", eval," -max_target_seqs ",max.hits," -num_threads ",cores," -dust no -outfmt '6 qseqid sseqid pident nident 
                   length mismatch gapopen gaps positive ppos qstart qend qlen sstart send slen evalue bitscore score'")
    )
    ...
    BLASTColNames <- c("query_id","subject_id", "perc_identity",
                       ...
                       "ppos", "q_start", "q_end","q_len","s_start","s_end",
                       "s_len","evalue","bit_score","score_raw")
    ...
    for (i in seq_len(length(full.te.chr))) {
        ...
        if ((length(ir.3ltr_ov_ir.full.te) > 0) & (length(ir.5ltr_ov_ir.3ltr) > 0) & (length(ir.5ltr_ov_ir.full.te) > 0)) {
            ...
            BLASTOutput_3ltr_chr <- BLASTOutput_3ltr_chr[-ir.3ltr_ov_ir.full.te@queryHits, ]
            ...
            BLASTOutput_5ltr_chr <- BLASTOutput_5ltr_chr[-c(ir.5ltr_ov_ir.full.te@queryHits,ir.5ltr_ov_ir.3ltr@queryHits), ]
        } 
        
        if ((length(ir.3ltr_ov_ir.full.te) > 0) & (length(ir.5ltr_ov_ir.3ltr) == 0) & (length(ir.5ltr_ov_ir.full.te) > 0)) {
            # exclude overlaps between 3ltr locus and full ltr transposon locus 
            BLASTOutput_3ltr_chr <- BLASTOutput_3ltr_chr[-c(ir.3ltr_ov_ir.full.te@queryHits), ]
            # exclude overlaps between 5ltr locus and full ltr transposon locus 
            BLASTOutput_5ltr_chr <- BLASTOutput_5ltr_chr[-c(ir.5ltr_ov_ir.full.te@queryHits), ]
        } 
